...
router.get(
    ...
    async (req, res) => {
        ...
        // Filter tasks to include only those with pending completion requests
        const pendingTasks = [];
        for (const task of tasks) {
            const taskCompletionRequest = await TaskCompletionRequest.findOne({ task: task._id, status: 'pending' });
